...
func (repo *DestinationRepo) CreateDestination(req *pb.AddDestinationRequest) (*pb.AddDestionationResponse, error) {
	...
	err := repo.DB.QueryRow(`
		INSERT INTO destinations (name, country, description, best_time_to_visit, average_cost_per_day, currency, language)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id, name, country, description, best_time_to_visit, average_cost_per_day, currency, language, created_at
...
func (repo *DestinationRepo) GetTravelDestination(id string) (*pb.GetDestinationResponse, error) {
	...
	err := repo.DB.QueryRow(`SELECT id, name, country, description, best_time_to_visit, average_cost_per_day, currency, language
		FROM destinations WHERE deleted_at = 0 and id = $1`, id).Scan(&resp.Id, &resp.Name, &resp.Country, &resp.Description, &resp.BestTimeToVisit, &resp.AverageCostPerDay, &resp.Currency, &resp.Language)
...
func (repo *DestinationRepo) GetDestinationActivities(id string) ([]string, error) {
	...
	rows, err := repo.DB.Query(`SELECT activity FROM destination_activities da INNER JOIN destinations d ON da.destination_id = d.id
		WHERE da.destination_id = $1`, id)
...
func (repo *DestinationRepo) GetDestinationAttractions(id string) ([]string, error) {
	rows, err := repo.DB.Query(`SELECT attraction FROM top_attractions WHERE destination_id = $1`, id)
...
func (repo *DestinationRepo) GetTrendingDestinations(limit int) (*pb.GetTrendDestinationResponse, error) {
	rows, err := repo.DB.Query(`
        SELECT id, name, country, popularity_score
        FROM destinations ORDER BY popularity_score DESC LIMIT $1
    `, limit)
	if err != nil {
		return nil, err
	}
	...
	var total int32
	err = repo.DB.QueryRow(`SELECT COUNT(*) FROM destinations WHERE deleted_at = 0`).Scan(&total)
